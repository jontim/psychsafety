...
The Sky Palace is on the chart as an inset: Jon's overview plate of the palace, bound into the sheet's corner as a framed panel (an entry in `insets` with `shape: "panel"` and an `image`), the three palace scenes pinned on it, the road walking between them and leaving by the private berth, where the Humā lies by the Caliphina's quarters. His other two plates, the berth and the dining terrace, are different angles of the same huge structure, so they are not more pins but the `view` a scene shows when it is boarded. The dining terrace, where it happened, is marked on the panel from the start (a `place` with an `inset`). The Congress is not in the palace: it sits on the land below, carved out of a mountain, and the Humā takes the Caliph down to it before the flight north. Nothing is discovered from inside an inset; names are found from the air and the road.
