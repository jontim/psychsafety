...
The geography is the world pack's to draw: `world.chart` holds the land as an SVG path, waters, ranges (drawn as chevrons), forests, region names, places, insets, a waypoint per beat and a glyph per ending; `validateWorld` refuses a chart that leaves a beat or an ending unplaced. The Shadow Fell's sheet follows the vault's ruled relationships (Halyra far south below the Antiquity, the Aegis Peaks between it and Omahnd, the Reach around the Inner Sea, Tharcia beyond the sea's northern arm and the Wall, Covalis north, Avarand far west); its coastlines, distances and the tour's stops are a picture, not a survey, and no town on the tour is named. Worlds without a chart get the role cards.
