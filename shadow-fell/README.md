...
The opening screen is the Scribe's chart: Jon's map of the continent, blank of names, with the Sky Palace bound into its corner and nothing else on it. The road is the player's and draws itself section by section as the story moves from one scene to the next, the way an old adventure film draws a line across a map, but only where the player actually goes: the sheet ahead stays blank, scenes not yet reached are not on it, and a road not taken is never drawn (decide it was not Omahnd and the Humā never goes there). Between scenes the chart is the first interlude: the road walks between the palace pins, the Humā takes the Caliph down to the Eightfold Hall, and the Shahin flies straight to the Reach when Navid is sent to hire deniables, lands and fades as the tour wagon grows in its place, and the wagon drives to the next challenge; the portrait and the place appear on arrival, and names appear as the vehicle crosses within reach of them. When an ending fires, its branch is drawn and the glyph lit before the ending footage plays. During play the Chart pill opens the road so far over the stage. Under the chart, scene chips let a tester jump in at any scene; the sheet then shows the plain road up to it. A world that sets `foreknowledge: true` gets the older behaviour instead: the road ahead faint, and the forks and endings shown on arrival.
...
The Sky Palace is on the chart as an inset: Jon's overview plate of the palace, bound into the sheet's corner as a framed panel (an entry in `insets` with `shape: "panel"` and an `image`), the three palace scenes pinned on it, the road walking between them and leaving by the private berth, where the Humā lies by the Caliphina's quarters. His other two plates, the berth and the dining terrace, are different angles of the same huge structure, so they are not more pins but the `view` a scene shows when it is boarded. The dining terrace, where it happened, is marked on the panel from the start (a `place` with an `inset`). The Congress is not in the palace: the Eightfold Hall is carved into Mount Sifr, the null mountain half a mile below it, so the palace's anchor sits beside the Congress pin; the Humā takes the Caliph down to it, and Navid's cutter, the Shahin, flies north from there (both ships are drawn with the one skyship sprite). Jon's sheet of the Eightfold Hall is that scene's view. Nothing is discovered from inside an inset; names are found from the air and the road.
