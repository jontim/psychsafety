...
export const ChartSchema = z.object({
  ...
  places: z.array(z.object({ id: z.string(), label: z.string(), at: PointSchema, glyph: z.enum(["palace", "city", "port", "pass", "site"]).default("city"), reveal: z.string().optional(), reach: z.number().optional(), inset: z.string().optional() })).default([]),
  ...
  /** A magnified view for scenes inside one building, drawn in `box` on the sheet: a circle inscribed in the box (the classic inset), or a panel carrying a plate of the place (`image`, fitted to the box, like an approach plate bound with the chart). Waypoints and places in it are placed in chart units inside the box; the road leaves it at `exit` and continues from `anchor`, the place on the sheet it magnifies. */
  insets: z.array(z.object({ id: z.string(), title: z.string(), box: z.tuple([z.number(), z.number(), z.number(), z.number()]), shape: z.enum(["circle", "panel"]).default("circle"), image: z.string().optional(), anchor: PointSchema, exit: PointSchema, plan: z.array(z.string()).default([]) })).default([]),
  /** A waypoint per beat. `by` names the vehicle that lays the road into it (none: on foot, no vehicle). `via` is the plain way in; `routes` are other ways, each taken when its flag is set, so the road can depend on what the player decided. `view` is a plate of the place shown when the scene is boarded. */
  waypoints: z.array(z.object({ beat: z.string(), at: PointSchema, place: z.string(), label: z.string().optional(), inset: z.string().optional(), via: z.array(PointSchema).default([]), routes: z.array(z.object({ flag: z.string(), via: z.array(PointSchema).default([]), label: z.string().optional() })).default([]), side: z.enum(["left", "right", "above", "below"]).optional(), by: z.string().optional(), view: z.object({ src: z.string(), caption: z.string() }).optional() })),
...
export function validateWorld(world: World): string[] {
  ...
  if (world.chart) {
    ...
    for (const pl of c.places) if (pl.inset && !insetIds.has(pl.inset)) problems.push(`Chart: place ${pl.id} names unknown inset ${pl.inset}`);
