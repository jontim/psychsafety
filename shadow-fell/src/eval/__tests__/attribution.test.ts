...
describe("the attribution eval", () => {
  ...
  it("hears Brask conjugate", () => {
    ...
    expect(slipsStyle("kael", "You need to move the horses.")).toBe(false);
    expect(slipsStyle("kael", "Write that down. A magistrate can read a horse.")).toBe(false);
    expect(slipsStyle("kael", "Sit down, your tea's going cold in your own hand.")).toBe(false);
    expect(slipsStyle("kael", "You should give him back.")).toBe(true);
    expect(styleSlipReasons("kael", "There's soup on the fire, a blanket by the door and bread in the sack. Eat.")).toEqual(["domestic tending: soup, blanket, by the door, bread, eat"]);
    expect(slipsStyle("kael", "Keep the loaf. Who's on that road?")).toBe(false);
