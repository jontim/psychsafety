...
export function systemPrompt(world: World, brief: string, dossiers: Record<string, string> = {}, runtime?: CanonRuntime, gate = true): string {
  return [
    ...
    ...(gate ? ["- Fill slate as the scene's paperwork: who owns the problem this turn, the coverage mode, how the Wardens read the outsider, and two to four candidate moves the speaker could make with this line, each scored against the company's runtime and marked distinct when no other Warden present could make it essentially unchanged. The line never renders a −2. When two or more moves score +1 or better, render the best-scored, and break a tie toward the distinct move, the one that arises from this Warden's way of knowing. When only one does, that is information about the character, not a failure: plurality is optional, specificity is mandatory. The move decides what the line does; the card, the dossier and the runtime decide how it sounds. The slate never replaces the character."] : []),
